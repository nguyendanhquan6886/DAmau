<?php
foreach ($result_pro_cate as $value) {
   $cateid = $value['cate_id'];
   $danhmuc = query_onecate($cateid);
}

?>
<h1 style="margin: 17px;"><?php echo "Sản Phẩm Danh Mục " . $danhmuc['cate_name'] ?></h1>
<div class="items">
   <?php

   foreach ($result_pro_cate as $procate) {
      extract($procate);
   ?>
